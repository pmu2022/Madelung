...
namespace lsms {
   ...
   */
  template<class T>
  T reciprocal_space_term(Matrix <T> &knlat,
                          std::vector<T> &knlatsq,
                          std::vector<T> &aij,
                          int nknlat,
                          double eta,
                          double omega) {
   ...
   */
  template<class T>
  T real_space_term(Matrix <T> &rslat,
                    std::vector<T> &aij,
                    int nrslat,
                    int ibegin,
                    T eta) {
 ...
 */
  template<class T>
  std::vector<std::complex<T>> dlsum(
      std::vector<T> &aij,
      Matrix <T> &rslat,
      int nrslat,
      int ibegin,
    ...
    for (
      ...
      for (
          ...
          kl > 0; kl--) {
        auto l = lofk[kl];
        dlm[kl] = dlm[kl] + gamma_l[l] * Ylm[kl] /
                            std::pow(vhalf, l
                                            + 1);
      }
