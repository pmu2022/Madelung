...
contains
   ...
   !
   !  ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
   subroutine printMadelungMatrix(iprint) bind(C, name = "printMadelungMatrix")
      !  ===================================================================
      ...
      do i = 1, LocalNumAtoms
         ...
         do j = 1, GlobalNumAtoms
            if (print_level > 0) then
               write(6, '(a,2i5,a,d20.13 )')' Madelung Matrix(j,i) for atom ', &
                  j, i, ': ', madmat(j, i)
            endif
            sum = sum + madmat(j, i)
         enddo
...
!
   ...
   subroutine dlsum(ibegin, aij, dlm)
      ...
      integer (kind = IntKind), intent(in) :: ibegin
      !
      integer (kind = IntKind) :: i, kl, l, lp1, m, j
      !
      real (kind = RealKind), intent(in) :: aij(3)
      !
      ...
      complex(kind = CmplxKind), allocatable :: ctmp2(:)
      ...
      allocate(ctmp2(1:kmax_mad), source = ctmp)
